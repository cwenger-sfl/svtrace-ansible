...
```bash
cqfd -b run_tests exec ansible subscriber_host -i inventories/qcom.yml -b -m shell -a 'docker run --detach --name sv_timestamp_logger_arm64 --privileged --cpuset-cpus 5 --volume /root/latency_tests:/tmp/latency_tests --device /dev/ptp0:/dev/ptp0 --network host --cap-add NET_ADMIN --cap-add SYS_NICE localhost/sv_timestamp_logger_arm64 -d end0 -f /tmp/latency_tests/ts_subscriber_host.txt -s 0000 --clock_device /dev/ptp0'
```
